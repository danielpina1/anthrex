...
| `i` | in the overview: show / hide the inspector panel |
...
The bottom bar shows the branch, uncommitted-work count, divergence from upstream, and any in-progress merge or rebase for the focused agent's checkout. A filesystem watcher gives the fast path, so it moves within a fraction of a second of a commit; a 30-second safety poll of every registered checkout is the backstop, so the cost while a repository sits idle is one cheap `git status` probe every 30 seconds, not nothing. The watch is recursive, and on Linux's inotify backend that takes one descriptor per directory in the checkout, so on a host with a low `fs.inotify.max_user_watches` the watcher can fail to arm and that checkout falls back to the poll alone — correct, just up to 30 seconds behind. Set `ANTHREX_GIT=off` to disable the watcher and probes entirely. The project tree (`C-b t`) is a sidebar list drawn with box-drawing connectors (`├─`, `└─`, `│`) at every level, to any depth, so a worker's own sub-agents stay legible. The overview (`C-b T`) draws the same tree as a graph instead: rounded boxes joined by edges, projects on the left and sub-agents to the right, on a canvas you pan with the wheel or a drag. `h`/`l` and `Left`/`Right` move the selection to a node's parent or first visible child, and the canvas follows the selection when it leaves the screen. Below the canvas the inspector panel spells the selected node out: a project's path, status and agent counts; an agent's model, status and tool, how long it has been in it, its checkout and that checkout's branch, and what it has running under it; a sub-agent's task in full, its state and — the one thing the tree cannot tell you — who spawned it, which is the parent sub-agent when it has one and the owning agent otherwise. `i` hides the panel and brings it back, and a terminal too short for it falls back to a single line, losing the panel and never the canvas.
