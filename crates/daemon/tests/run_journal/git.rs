...
impl World {
    ...
    /// Task `task`'s checkout on `branch` from `from`, its own repository in the run's
    /// data directory as the daemon makes it (final fix batch F1c); its `HEAD`.
    pub fn prepare_task(&self, task: &str, branch: &str, from: &str) -> String {
        let path = self.run.task_path(task);
        daemon::run::git::prepare_task_worktree(
            real_git(),
            self.root(),
            branch,
            from,
            &path,
            &daemon::run::git::checkout_repo_dir(&self.run.data_dir, &path),
            T,
        )
        .unwrap()
    }
    ...
    pub fn task_with_commit(&self, task: &str, file: &str, content: &str) -> (PathBuf, String) {
        ...
        self.prepare_task(task, &branch, &self.run.base_sha);
...
fn reconcile_prepare_worktree_done_and_not_started() {
    ...
    w.prepare_task("t1", &t1_branch, &base);
    w.prepare_task("t2", &t2_branch, &base);
    // A checkout whose making died part way (final fix batch F1c): its directory and
    // repository made, its files never put in place (no `ready` marker).
    let partial = w.run.task_path("t3");
    write(&partial, "half-written.txt", "x");
    std::fs::create_dir_all(
        daemon::run::git::checkout_repo_dir(&w.run.data_dir, &partial).join("git"),
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        "in place on its branch: replayed; with a setup: re-run; partial, missing or \
         on another branch: not started"
    );
    // A partial checkout is left for the re-issued op, which finishes it (decision 43):
    // nothing in it is removed.
    assert!(partial.join("half-written.txt").is_file());
    assert!(
...
fn reconcile_remove_worktree_gone_with_salvage_ref() {
    ...
    assert!(t3.is_dir());
    // Final fix batch F1c: each checkout's own repository goes with it (the worker's
    // private objects included); one a crash left behind is removed by reconcile.
    let repo_of = |path: &Path| daemon::run::git::checkout_repo_dir(&w.run.data_dir, path);
    assert!(!repo_of(&t1).exists(), "the removal left its repository");
    assert!(!repo_of(&t2).exists(), "reconcile left the repository");
    assert!(repo_of(&t3).join("git/HEAD").is_file());
}
