...
//! or with the run paused or halted, the task's clock is stopped (`Task.clock_stopped`); when both hold again,
...
pub(super) fn sync(run: &mut Run, now: u64) {
    ...
    for task in run.tasks.iter_mut() {
        ...
        match task.clock_stopped {
            None if !charging => task.clock_stopped = Some(now),
...
/// downtime before the restore is no session time either.
pub(super) fn stop_at_restore(task: &mut Task) {
    if task.state != TaskState::Working || task.clock_stopped.is_some() {
        return;
    }
    let alive = worker_round(task).map(|r| {
        let round = &task.rounds[r];
        round.last_event.max(round.started_at)
    });
    task.clock_stopped = alive;
}
...
pub(super) fn epoch_spend(task: &Task, now: u64) -> Spend {
    ...
    let secs = task
        ...
        .map(|(_, round)| round_spend(round, task.clock_stopped, now).secs)
        .sum();
    Spend {
