...
- **Descriptor exhaustion is a separate risk from that CPU one, and none of those three defences touch it.** The filter, the debounce and the breaker all act on events that have already been delivered; the watch is registered recursively, and on inotify that costs one descriptor per directory in the tree, ignored directories included — a Rust checkout with a populated `target/` is tens of thousands. Past a host's `fs.inotify.max_user_watches` the watch fails to arm at all and the root drops silently to the 30-second poll, and §5's one-worktree-per-agent multiplies the count by the number of agents. The fix is a non-recursive watch plus a pruning walk, recorded against milestone 6 in `docs/superpowers/plans/2026-09-17-anthrex-foundation-followups.md` because it shares its filter with that milestone's `[git]` `ignore` list.
