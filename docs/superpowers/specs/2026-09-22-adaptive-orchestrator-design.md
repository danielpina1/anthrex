...
- A **chain** of tasks where each depends only on the previous one, and whose combined size is still M, is one task: each extra link adds a worker cold start, a check, a review and a merge, and nothing runs beside it. [J, following the sequential-work penalty in the scaling study] This is a planning rule, written into the orchestrator's and sub-planners' contracts. The engine's own plan validation (§7.2) has no chain rule, so plan files and the user's edits may keep chains; only batches from the orchestrator and sub-planners are also checked for it (weights S = 1, M = 3, at most 3 in all). *(settled in the briefs, 2026-09-22)*
...
- **The base branch is guarded twice.** At every merge step and before `Complete`, the base and run refs must equal the engine's recorded values, or the run halts. A `reference-transaction` hook rejecting writes to protected refs is an optional second layer; it can likely be bypassed, which is why the ref check is mandatory. [E]
...
A green S task on the fast path · a TDD task whose red commit does not fail (proof rejected) · a TDD task whose test passes without the named test running · a task that fails check once, then passes · a stall → rung 2 on the peer runtime · a mis-sized task → split by the orchestrator · a merge conflict → hand-back → resolved · a second conflict → blocked · a candidate merge that is red although both branches were green · a cancel of a running task with dirty work (salvaged) · a ref moved behind the engine's back (run halts) · a daemon killed after each logged intent (reconcile lands in the same state) · a rate-limit event halving writers · a race whose loser is stopped and salvaged · a test writer's red commit handed to an implementer · a review rejected twice → fresh worker on the peer runtime → approved · a worker that disputes a finding (`task_blocked: question`) · a user override merged without approval, marked in the report · input, kill and terminal subscribe to a headless window refused by the daemon while engine messages still arrive · a rate-limit retry in the stream (rate_limited, not a finished turn) · repeated permission denials → blocked(environment) · a headless process that dies mid-turn → resumed · every session resumed after a daemon restart · a plan edit that leaves an L task (rejected) · a sub-planner edit outside its area (rejected) · Claude and Codex given overlapping `owns` (rejected).

## 22. Decisions for the author

1. **Headless agents and deciders on the user's login.** Every non-orchestrator agent and every decider runs `claude -p`. `claude -p` without `--bare` uses the user's subscription; Anthropic's policy forbids third-party products *offering* claude.ai login, and a local tool running the user's own binary is likely fine but not confirmed. If not acceptable, deciders fall back to their deterministic defaults or to Codex.
2. **S review default.** On (cheap, honours the roadmap) as written, or off.
3. **`planner_task_cap`.** 12 as a starting point, beyond which the large path and sub-planners take over. No source measures the right number.
