...
### Final review create-timeout correction

- Whole-branch review found an integration race between the daemon's five-second project detector and the one-shot CLI's identical five-second reply deadline: `anthrex new` could report a timeout just before the detached server task created the window. The existing `CliClient::request` keeps its five-second deadline; its receive loop is shared with an explicit-timeout variant used only by `Command::New`. `CreateWindow` receives `project::DETECT_TIMEOUT` plus an explicit two-second allowance for timeout cleanup, task scheduling, PTY spawn, and the reply; every ordinary command retains the existing five-second deadline, and the hook command remains unchanged.
- A real isolated CLI regression injects an owned `git` executable through the daemon's `PATH`. The wrapper records its exact PID and sleeps beyond `DETECT_TIMEOUT`. Before the correction, `anthrex new` exited 1 with `timed out waiting for the daemon` while the server subsequently created the window. After the correction, the command prints the one created id, the daemon records exactly one window with the canonical-cwd fallback project, the timed-out Git PID is gone, and removing that window leaves no duplicate or leaked agent process. Test support gained only a daemon-command configuration hook and an explicit per-invocation outer deadline for this slow control.

### Final audit corrections

- The final safety/specification audit found that decision 5's literal read-after-exit implementation did not actually bound the whole detector: a descendant that inherited Git's stdout kept `read_to_end` blocked after the direct child exited. Project detection now makes the pipe nonblocking and drains it inside the original deadline, with deadline checks inside the drain and a 64 KiB output cap. A successful result still requires both a successful direct-child status and pipe EOF. At the deadline, a live direct child is killed and reaped; an already-reaped child with an inherited writer is handled by dropping the pipe and returning the canonical-cwd fallback. A real executable regression owns and explicitly terminates the exact helper PID that holds stdout open; it failed on the blocking drain and returns within the deadline after the correction. No detached reader thread or new dependency was introduced.
- `tree --project` now retains both the canonical requested directory and its Git-normalized root. The shared pure selector chooses an exact recorded root for the request first, then the longest recorded root containing it, and only then applies the same exact/longest precedence to the normalized root. This preserves the daemon's immutable project identity if repository topology changes after window creation, while still mapping an external sibling linked worktree to the recorded main-checkout project. A process regression creates a window in a plain `repo/sub`, initializes Git at `repo`, creates a second `repo` window, and verifies that filtering `repo/sub` returns only the original project; the existing sibling-worktree regression covers normalized fallback. Connecting before resolving the optional directory remains unchanged.

### M4.1–M4.4 foundation and harness history

- The Ubuntu smoke failure after stages 1–6 was a harness focus race, not an Alt+Enter encoding defect. A `WindowsChanged` list update can render a newly created shell in the sidebar before the independent `Created` reply focuses it. The former `wait_for("shell-3")` therefore allowed a command's early per-key input events to target the old shell while the later events targeted shell-3. The supplied framed-socket proxy deterministically held `Created(3)` after `WindowsChanged`: it observed `ca` to shell-1 and `t -v` to shell-3, matching the CI screen and failure. Every smoke creation boundary that immediately sends input (stages 2, 3, 7, and 8) now waits for the focused main-pane title (`shell-N · shell`), which is not satisfied by sidebar membership. Stage 7 also replaces its fixed post-command sleep with `stty -echo; printf '%s%s\\n' CAT_ READY; cat -v` and waits for the split-output readiness marker before sending ESC CR; this proves the complete command executed in the focused child without accepting echoed command text as readiness. No production or protocol behavior changed.

- M4.2 added a `tokio::task::spawn_blocking` boundary around `WindowManager::create` after asynchronous project resolution. The brief's shorthand said to call `manager.create`, but that call performs PTY process spawn and file work, so running it on a Tokio worker would violate the repository's blocking-work rule. The pre-existing manager-internal spawn-under-lock debt remains assigned to M5.
- Review of M4.2 exposed a shutdown race in which a detached slow creation could finish after shutdown had taken its cleanup snapshot. `WindowManager` now closes creation admission under the same mutex before collecting that snapshot; a creation admitted first is included, while one arriving after shutdown is rejected before it can spawn. A signal-coordinated regression covers this ordering without using sleeps as its synchronization.
- The CLI integration harness originally captured child stdout/stderr through reader-thread pipes. A deterministic real-shell regression showed that a descendant holding those descriptors could outlive a successfully exited direct child and make the harness wait for EOF. Commit `3ffaaf2` changed the harness to file-backed capture with a bounded direct-child wait. This was a verification prerequisite rather than product behavior, and repeated startup-probe/full-workspace checks passed after it.
- M4.3's initial tree implementation required review hardening beyond the task's first pass: display-name collision resolution now runs globally across basename groups, short-model truncation preserves complete Unicode grapheme clusters within eight terminal columns, and its tests moved to `tree/tests.rs` to keep production and test files focused. M4.4 placed its collapse/filter/selection/viewport tests in `tree/tests/state.rs` for the same file-size reason; the public pure-tree API was unchanged by those test-module splits.
