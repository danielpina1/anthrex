...
- **I2: checks and proofs run confined.** When the run's workers are sandboxed
  (`[orchestrator] worker_sandbox`, decision 54), every `check` (a task's done check and
  a scratch check), both proof runs and the integration candidate's check run under a
  `sandbox-exec` profile (`run/confine.rs`) that denies every file write except to:
  the checkout it runs in; that checkout's own object store when it is its own
  repository (3a); its per-checkout temporary directory
  (`<data>/runs/<run>/tasks/<name>/tmp`, which is its `TMPDIR`); the profile's new
  `cache_dirs`; and `/dev`. The user's `.git` (the integration checkout's git dir
  included), their checkout and `$HOME` are read-only. Each path is spelled as I1's
  grant is (the parent resolved, the leaf appended and checked with `lstat`); a checkout
  that cannot be confined, or a `cache_dirs` entry that overlaps the git common
  directory or anthrex's data directory, fails the command unrun (fails closed).
  - `sandbox-exec` applies the profile and `exec`s `/bin/sh`, so the pid and process
    group the engine waits for and kills are the shell's as before; none of the wait
    or kill code changed (`a_confined_command_that_times_out_is_killed_with_its_group`).
  - `profile.cache_dirs` (plan `[profile]` and `[orchestrator.profile]`, the plan
    winning as for every key but `protected`): absolute, `~/…` (the daemon's `$HOME`),
    or relative to the checkout; `..` is refused. It is read from TOML only, never sent
    on the wire (a plan travels as `plan_toml`), so `PROTO_VERSION` is unchanged; a run
    recorded before F1c reads it as empty.
  - **Deviation from the dispatch's suggestion:** no `CARGO_TARGET_DIR` default. The
    checkout is writable, so cargo's default `target/` works, and cargo builds and tests
    with a read-only `$CARGO_HOME` once its crates are fetched (probed: `cargo build`
    and `cargo test` of a crate depending on `libc`, under the same profile, with
    `~/.cargo` read-only). A default the profile's `env` did not ask for would change
    where a project's own scripts find their build.
  - **Costs, recorded:** a check that writes outside its checkout (a global cache, a
    socket under `/tmp`, `~/.npm`) fails until the profile lists that directory in
    `cache_dirs`. `setup` is not confined (it runs the profile's command before the
    worker's code at a task's `from`, and usually needs the network and the user's
    caches); at a proof's scratch, `setup` runs at `red`, which holds the worker's test
    commit, so a worker-written `package.json` script could still run unconfined there.
    Recorded as a follow-up.
  - **Linux:** the engine has no confinement it can rely on there (`confine::AVAILABLE`
    is false), so checks and proofs run unconfined, as before F1c. Recorded in manual
    check 4e and the follow-ups; a bubblewrap or landlock wrapper is the fix to weigh.
  - **Tests.** `run_confine.rs` (a confined check's writes to the user's object store,
    index and `$HOME` are denied while its checkout, `TMPDIR`, own objects and a cache
    are written; the integration check cannot write its git dir; a confined proof; the
    timeout kill; a cache over the repository refused), `run_e2e_confine.rs` (through a
    real daemon: a plan check that writes `<common>/objects/pwned`, `<common>/index` and
    `$HOME/pwned` leaves all three untouched and the run completes; red on `4773b24`
    with "a check wrote the user's object store"), `confine.rs`'s unit tests, and the
    config and plan tests for `cache_dirs`. `run_e2e_basic.rs`'s
    `e2e_a_failed_merge_candidate_reattaches_the_integration_worktree` had its check
    write the base ref to fail the candidate; a confined check cannot, so the check now
    waits at the candidate while the test moves the base from outside. The same for
    `run_e2e_base.rs`'s D-7 test, whose check left an `index.lock` in the integration
    worktree's git dir (the test now leaves it while the check waits), and
    `run_e2e_review.rs`'s override test, whose check logs where it ran to a file now
    under a `cache_dirs` entry.

#### F1c round 2 (2026-09-25)

- **`setup` runs confined.** `setup` runs code the workers can reach: the repository's
  own script, and at a proof's scratch checkout the worker's `red` commit. It now runs
  under the same confinement as checks and proofs wherever it runs (the integration
  checkout, each task checkout, a scratch check's checkout, the proof checkout).
  Tests: `run_confine.rs`'s `a_worker_committed_setup_at_the_proof_checkout_is_confined`
  and `run_e2e_confine.rs`'s `e2e_setup_cannot_write_the_users_git_or_home` (both red
  on `2236042`). Cost: a `setup` that fills a package manager's cache under `$HOME`
  needs that directory in `cache_dirs`.
- **No silent unconfined checks.** Where the platform cannot confine checks, proofs and
  `setup` (Linux; `confine::available()`), `run start` refuses a run whose workers are
  sandboxed unless the user passes `anthrex run start --unconfined-checks` or sets
  `[orchestrator] unconfined_checks = true` in their own `config.toml` (never the plan
  or its profile). The run records it (`RunLimits::unconfined_checks`, serde default
  `false`); `run status` prints `checks: unconfined (...)` and the report prints
  `checks, proofs and setup: unconfined (...)`. The flag never turns confinement off
  where it is available. A run with `worker_sandbox = false` is not refused (that is
  already the user's explicit choice); its report now also says its checks are
  unconfined.
  - Wire: `RunRequest::Start::unconfined_checks` and `RunInfo::unconfined_checks`,
    both `#[serde(default)]`, with the round-trip test pinning the new field.
    `PROTO_VERSION` stays 7: version 7 is new on this unmerged branch and has not
    shipped, as M6.5.10 (a field added to an unshipped 6) and M8a's `7e8a947` did.
  - The daemon's `ANTHREX_CHECK_CONFINEMENT=unavailable` makes it treat the platform
    as unconfinable, so `run_e2e_unconfined.rs` (the refusal from the request and the
    CLI, the flag, the config key, and on macOS that the flag leaves confinement on)
    runs on macOS. It can only produce a refusal or what the flag allows.
...
#### F1d: confinement rebuilt on a deny-by-default base (2026-09-25)

Re-review 2 (`final-fix-F1c-rereview-2.md`) found that three rounds of deny-listing on an
`(allow default)` base each left a route to an unconfined actor open (R1: `open` of a
check-built `.app`, `defaults write`; R4: the keychain and localhost services). Ruling
F1d: rebuild the profile deny-by-default.

- **R1 (Critical): the confined profile is `(deny default)` with an explicit allow-list**
  (`run/seatbelt.rs`, modelled on Codex's `seatbelt_base_policy.sbpl` from codex-cli
  0.156.1 and Claude Code 2.1.280's sandbox-runtime policy, both read from the installed
  binaries). Allowed: `process-exec`/`fork`; `signal` and `process-info*` only
  `(target same-sandbox)` (each `sandbox-exec` is its own instance, verified: a check
  cannot signal a user process nor another check's; SBPL has no process-group target, so
  "own process group" is approximated by "own sandbox instance", which is at least as
  narrow for the user's processes); `file-read*` everywhere except other terminals;
  `file-write*` only under the writable set and `null`, `zero`, `random`, `urandom`,
  `tty`, `dtracehelper`, `/dev/fd`; `sysctl-read`; POSIX semaphores and shared memory
  (writes never to `apple.*` names); `user-preference-read`; `iokit-open` of
  `RootDomainUserClient`; the vnguard and sandbox-container `mac-syscall`s; Unix-socket
  bind/connect only under the writable set; and the Mach services
  `seatbelt::MACH_SERVICES` lists with the reason for each (opendirectoryd libinfo and
  membership, DirectoryService libinfo, `bsd.dirhelper`, `system.logger`, `logd`,
  `diagnosticd`, `system.notification_center`, the two `cfprefsd` endpoints for preference
  reads). Everything else is denied by the base, notably `lsopen`,
  `user-preference-write`, `appleevent-send`, `com.apple.SecurityServer`,
  ...
  with `open -g -n`; unconfined it ran under launchd and wrote the marker; unregistered
  with `lsregister -u` by its exact path afterwards),
  `a_confined_check_cannot_write_preferences` (`defaults write` to a unique
  `com.anthrex.f1d-test.prefs.<pid>` domain; deleted by exact name afterwards),
  `a_confined_check_cannot_reach_the_keychain_launchservices_or_appleevents` (a ctypes
  `bootstrap_look_up` of each service: found unconfined, denied confined; no keychain is
  read or created). The round-3 N1 tests (`run_confine.rs`) still pass. The stale claim
  that the daemon socket lives under the data directory (re-review 2, M1) is gone from
...
- **R2 (Important): PTYs work.** `(allow pseudo-tty)`, `/dev/ptmx` read/write/ioctl, and
  ...
  other `/dev/ttys*` is denied for reads and writes, so a check can neither write to nor
  read keystrokes from the user's terminals. Tests: `a_confined_check_can_open_a_pty`
  (`pty.openpty()` round trip and `script -q /dev/null`), red on `6f0e852`;
  `a_confined_check_cannot_open_another_terminal` (skips when the user has no terminal
  open). **Residual (Minor):** a check's own PTYs are writable by it, which is harmless;
  `file-ioctl` on another terminal is denied too, so `TIOCSTI` is out (it would also need
  that terminal as the check's controlling tty, which a `setsid` check never has).
- **R3 (Important): `cache_dirs` entries are absolute and resolved without following a
  link a run could have planted** (`run/confine_cache.rs`). The config refuses a relative
  entry at load with a problem naming it ("not an absolute path; each entry must start with
  / or ~/"), and drops that repository's entry. At use each component is `lstat`ed: a link
  inside the run's checkouts (`<wt>/runs/<run>`), the task temporary root, anthrex's data
  directory, or another `cache_dirs` entry, or a link at the entry itself, is refused
  ("name the directory it points to instead"); a link elsewhere (`/tmp`, `/var`, one the
  user made) is followed. Then the `$HOME`/dotfile/`~/Library` guards and the overlap
  checks (git common dir, data dir, the task temporary root, the run's checkouts, and an
  entry holding the daemon's socket, re-review 2's M3) run on the resolved path, and two
  entries that overlap are refused. Deviation: a `cache_dirs` entry that is itself a
  symbolic link the user made is refused too, because a check granted that entry could
  have made it; the user names its target instead. Tests: `confine_cache.rs` unit tests
  (a checkout component swapped for a link, a link inside another entry, a link at the
  entry, a user link followed and the result checked, every overlap),
  `a_cache_dir_through_a_planted_link_is_refused` through `ConfineSpec::for_checkout`, and
  `a_relative_cache_dirs_entry_is_refused_at_load`.
- **R5 (Important) and M2 (Minor): every worker and every confined command gets a short
  per-task `TMPDIR`, never the daemon's.** The task temporary directory moved from
  `<data>/runs/<run>/tasks/<task>/tmp` (86 to 115 bytes under the default data dir, so a
  `tempfile` Unix socket overflowed `sun_path`) to `/private/tmp/ax-<uid>/<16 hex>`
  ...
  its writes are denied). Worker sandbox settings are pinned on the command line so the
  user's `~/.claude`/`~/.codex` settings cannot widen them: Claude's `--settings`
  sandbox block adds `network.allowedDomains = []` (which also turns on Claude Code's
  network restriction, off when the key is absent), `network.strictAllowlist = true`,
  `network.allowUnixSockets = []`, `network.allowAllUnixSockets = false`,
  `network.allowLocalBinding = false`, `network.allowMachLookup = []`,
  `allowAppleEvents = false`, `enableWeakerNetworkIsolation = false`,
  `enableWeakerNestedSandbox = false`, `excludedCommands = []`, `filesystem.disabled =
  false` (keys read from Claude Code 2.1.280's settings schema; `SandboxKeys::pins`);
  every Codex session adds `-c sandbox_workspace_write.network_access=false`,
  `exclude_tmpdir_env_var=true` and `exclude_slash_tmp=true` (`CODEX_SANDBOX_PINS`), so its
  writable roots are exactly the checkout and its grant (objects, the task's `TMPDIR`,
  the commit's git files). Reviewers get the same pins. Tests: `argv_tests`
  (settings and argv), `role_launch`'s and `dispatch`'s `TMPDIR` assertions, and
  `run_e2e_worker_pins.rs` through a real daemon and `fake-agent` (the workers' recorded
  argv and the `TMPDIR` their shell saw; red with the old worker environment, which gave
  them the daemon's `$TMPDIR`). Whether a user-level array (for example
  `allowUnixSockets`) is replaced or merged by the `--settings` array is the real CLI's
  behaviour, checked in manual check 4e.
- **Linux: unchanged.** `AVAILABLE` stays false; `run start` still refuses a sandboxed run
...
### Final fix batch F2, round 2 (2026-09-25)

The F2 review (`final-fix-F2-review.md`) confirmed C-I1 to C-I4 and M2 to M5, and found
N1 (Important) and N2 to N5 (Minor). The coordinator also adopted the review's item 2: a
sandbox-level deny as defence in depth for the C-I1 race.

- **N1: shell and interpreter start-up inlets are reserved.** `config::reserved_env`
  now also refuses in a profile's `env`:
  - the names `BASH_ENV`, `ENV`, `ZDOTDIR`, `SHELL`, `SHELLOPTS`, `BASHOPTS`, `PS4`, `IFS`,
    `CDPATH`, `BUN_OPTIONS`, `NODE_PATH`, `JAVA_TOOL_OPTIONS`, `_JAVA_OPTIONS`, `EDITOR`,
    `VISUAL`, `PAGER`, `LESSOPEN`, `LESSCLOSE`, `SSH_ASKPASS` and `SUDO_ASKPASS`;
  - the families `BASH_FUNC_`, `PYTHON` (so `PYTHONPATH`, `PYTHONSTARTUP`, `PYTHONHOME`),
    `PERL5`, `RUBY` (`RUBYOPT`, `RUBYLIB`), `NPM_CONFIG_` and `XDG_`. `GIT_EDITOR` was
    already covered by `GIT_`.
  - Claude Code 2.1.280's own list of unsandboxed-exec inlets is the floor.
  - None is kept off. Every one has a way out: a check can set the variable in its own
    command (`PYTHONPATH=src pytest`), and a user-level value comes from the daemon's
    environment. The cost is recorded in the follow-ups.
  - The inherited `BASH_ENV`, `ENV`, `SHELLOPTS`, `BASHOPTS`, `PS4`, `IFS`, `CDPATH` and
    exported shell functions (`BASH_FUNC_*`) are also scrubbed from every session and
    engine command.
  - A user's own `ZDOTDIR`, `SHELL`, `PYTHONPATH` or `XDG_*` is kept. It is their own
    setting; only a profile may not set it.
  - The match is case-insensitive on purpose (lower-case proxies), so `path` or `home`
    are refused too.
  - Tests: `reserved_env`'s list test, `plan_tests_env.rs` (a plan with `BASH_ENV`
    refused), `headless_env.rs` (inherited `BASH_ENV`, `SHELLOPTS` and `BASH_FUNC_x%%` gone
    from Claude and Codex sessions). The scrub assertions were red before.
- **N2: the credential scrub covers every spawn path.**
  - Codex: anthrex has no Codex auth setting, and its Codex sessions use the user's own
    `codex login`. So every session loses `OPENAI_API_KEY`, `CODEX_API_KEY` and
    `CODEX_ACCESS_TOKEN` (`OPENAI_CREDENTIALS`).
  - Claude: sessions still keep the Anthropic keys only under `auth = "api_key"`.
  - Engine commands: checks, proofs and `setup` (`run::exec::engine_env`, the one spawn
    every engine command goes through) lose every API credential.
  - Tests: `headless_env.rs`, `run_exec_env.rs` (through `run_shell`, `run_confined` and
    `confine::confined`), and `credential_scrub`'s unit test. The scrub assertions were
    red before.
  - Cost: a user who authenticates Codex only through an exported key must use
    `codex login --with-api-key` instead. Recorded in the follow-ups.
- **Item 2: a sandbox deny on the protected agent-config paths.**
  - **Checks, proofs and setup:** the F1d profile appends, after its write allows, an
    unconditional `(deny file-write* ...)` for the checkout's `.claude` and `.codex` (the
    directories too), `.mcp.json`, and `CLAUDE.md` and `AGENTS.md` at any depth (an SBPL
    regex, with the checkout's path escaped) (`seatbelt::protected_denial`).
    `run_confine_protected.rs` checks it under real `sandbox-exec` (it was red before):
    - creating, writing, renaming or symlinking any of them fails, a hard link to a
      protected file and a write through it included (in any letter case, and nested
      files outside dependency trees only, since F4);
    - other files of the checkout stay writable.

    A check or `setup` that writes one of these files, such as a generator of
    `CLAUDE.md`, now fails. Deviation, recorded.
  - **Claude sessions:** `ClaudeSandbox.deny_write` becomes Claude Code's
    `sandbox.filesystem.denyWrite` (`SandboxKeys.write_deny`). The entries are the
    checkout's `.claude`, `.codex`, `.mcp.json`, `CLAUDE.md`, `AGENTS.md`, `**/CLAUDE.md`
    and `**/AGENTS.md` (`role_launch::protected_write_denials`).
    - A worker drops an entry when its `owns` names a path under it literally
      (decision 56's rule). A deny cannot carve out one file, and the done gate still
      judges the change. Owning `docs/AGENTS.md` drops the `**/AGENTS.md` glob, and
      owning `CLAUDE.md` drops `**/CLAUDE.md` too, since that glob also matches the root
      file.
    - A reviewer denies all seven.
    - Only `Bash` and its children are covered: Claude's `Edit`/`Write` tools are
      governed by permissions, not the sandbox, and the done gate covers them.
    - Tests: `argv_tests::deny_write_becomes_filesystem_deny_write`,
      `role_launch::claude_sessions_deny_writes_to_protected_agent_config`, and the
      argv assertion in `run_e2e_worker_pins.rs`.
    - Whether the real CLI honours the entries, the globs in particular, is manual check
      4e, F2 round 2 (5).
  - **Codex sessions: not added.** The legacy `sandbox_workspace_write` settings have
    no deny list, and a 0.156 permission profile may not be able to grant F1c's roots
    outside the checkout. The exact probe is manual check 4e, F2 round 2 (6).
- **Leftover worker children (`setsid`): recorded, not changed.** This is the
  process-kill safety rule. The follow-ups hold the evidence. The two denies above close
  the race for sandboxed writers; the Codex guard still catches config that arrives by
  merge or hand-back.
- **N3:** the follow-ups' C-I1 residual entry is rewritten: Claude and check denies are
  done, and the Codex route (permission profiles, with the "writes outside the workspace
  root" caveat) is left open.
- **N4: done, by message.** A tracked `.codex` file that the checkout converts (an
  `eol=crlf` or filter attribute) still refuses Codex (fail closed). The refusal now says
  that a line-ending or filter conversion also counts as a difference. Test:
  `a_converted_tracked_file_is_refused_with_the_reason_named`.
...
### Final fix batch F4: parked items, surface minors, F2/F3 re-review findings (2026-09-25)

Scope: `final-fix-plan.md` Part F4, the F2 re-review's I1, I2 and minors, and the F3
review's N1–N7 and B-8. Verification per the user's policy: build, clippy, fmt and the
touched test targets only (named in the F4 report).

- **F2 re-review I1 (case).** Not reproducible on macOS 26.2: `sandbox-exec` with the
  old profile already denied `.CODEX`, `.Codex`, `agents.md`, `Claude.MD`,
  `sub/Claude.md` and `.MCP.JSON` (probed directly with `literal`, `subpath` and `regex`
  filters on the default case-insensitive APFS volume; the kernel folds case). The new
  test `a_confined_command_cannot_write_protected_agent_config_in_another_case` so
  passed before the change; it pins the property. The confined profile now also spells
  every protected name with bracket classes (`seatbelt::any_case`), so the property
  does not depend on the kernel's folding. Claude's entries stay plain paths (no bracket
  entries were added): Claude's macOS sandbox is seatbelt, whose `subpath`/`literal`
  rules folded case in the probe; manual check 4e (7) confirms it with the real CLI.
- **F2 re-review I2 (dependency trees).** The confined profile's nested
  `CLAUDE.md`/`AGENTS.md` deny is now `(require-all <nested> (require-not
  ...
  **Deviation (Claude sessions):** Claude's `denyWrite` globs cannot exclude a tree, so
  ...
  Claude session denies the five root paths only. What still protects nested files: the
  done gate (any depth, any case), and Codex reads `AGENTS.md` only from the root to its
  cwd. Recorded in the follow-ups; manual check 4e (8).
- **F2 re-review M1.** A bare `.claude`/`.codex` in `owns` no longer lifts that
  directory's deny: only a literal strictly under it does (`names_literally` gives a
  bare directory no file). Test: `claude_sessions_deny_writes_to_protected_agent_config`.
  The anthrex profile does not deny create/unlink on the ancestors of nested protected
  files, so `mv docs docs2` relocates a tracked `docs/AGENTS.md`; the done gate sees the
  deletion.
- **F2 re-review M3.** `PYTHONUNBUFFERED`, `PYTHONDONTWRITEBYTECODE`, `PYTHONHASHSEED`,
  `PYTHONUTF8` and `PYTHONIOENCODING` (value-only) are allowed in a profile's `env`;
  `PYTHONWARNINGS` is not (a category names a module). `PAGER` stays reserved (a
  command). M2 and M4: follow-ups.
- **F3 review N1 (Important).** A started hub task held on an unfinished dependency it
  gained (`awaiting_deps`) no longer counts in `hub_started`; while it is held, only the
  tasks it waits for, transitively (`schedule::held_hub_waits_for`), may dispatch, so
  the hub is alone again when it resumes. Test:
  `a_hub_task_held_on_a_new_dependency_lets_that_dependency_run` (red before: t4 never
  dispatched). Consequence of decision 41, now said: a started hub in
  `blocked(mis_sized)` or `blocked(environment)` idles the rest of the run until the
  user acts.
- **F3 review N2.** A pid-0 synthetic exit is never a repeat (`signals.rs`), so a second
  kill of a reopened round ends it. Test: `a_second_synthetic_exit_ends_a_reopened_round`.
- **F3 review N3.** `guard::prepare_guarded` runs `effects::prepare` (the snapshot)
  under `catch_unwind` in the event loop and at restore; on a panic the effects are
  prepared again without their publishes. Test:
  `a_panicking_snapshot_drops_only_the_publish`.
- **F3 review N4.** The restore's `as_loaded` is taken before its log notes, so a run
  whose only change is a note is saved. No test: a note on an otherwise unchanged run
  comes from the leftover-session killer (process code) or a git failure that also
  changes the run's pending ops.
- **F3 review N5.** The panic test removes the poisoned run before its second request
  and asserts the engine's own answer. N6 (B-8) and N7: follow-ups.
- **T12-P1.** `a_count_after_a_failed_resume_is_dropped` asserts that the failed resume
  itself clears its round's awaited count (checked: removing `ladder::supersede` from
  that path turns it red).
- **T14-P1.** `GIT_NO_REPLACE_OBJECTS` is set only on the engine's own git calls
  (`subprocess::scrub_git_env`); checks, proofs, `setup` (`exec::engine_env`) and
  headless sessions (`session_env`) drop only rule 11's location variables
  (`scrub_git_location_env`). The daemon's read-only probes (`project.rs`,
  `git/probe.rs`) keep it: they go through `subprocess::run`, and not reading replace
  objects there is harmless.
- **T15-P1.** The liveness oracle lets a halted (or paused) run's `proof`/`check` task
  wait for the resume.
- **T16-P1.** `report_escape::plain_text_line`: the line after a blank line starts a new
  paragraph at column 0 (normalized, so it opens no block) instead of a 4-space line
  that was an indented code block; runs of blank lines are one.
- **T18.** N-1 (an interrupt in the spawn-to-install gap ends the session, acting as a
  kill) and N-2 (an interrupt during resume's retire is lost) stay as ruled: the gaps
  are milliseconds and the engine interrupts only on cancel. N-4: test
  `a_replaced_process_keeps_the_texts_sent_and_not_yet_prompted` (red with
  `process_replaced` clearing `pending_sent`).
- **T20.** The fake's `TOOL_REJECTED` is the recorded text in full; the stale 120 s line
  in M8a.20's notes is corrected; `headless_support/mod.rs` is split (below). P1, P2 and
  the unbounded `shell()` join are fake-agent signal and kill code: follow-ups.
- **T21-P1..P3.** Process-scan code, not changed: follow-ups.
- **T22.** P1: an edit refused over project settings gets its own text ("… start a new
  run with --trust-project"). P2: `run edit` probes git only for a batch that adds,
  splits or amends a task (`reach::edits_may_widen`; test
  `only_task_edits_may_widen_the_reach`). P3: `reconcile/mod.rs`'s comment corrected,
  and a replayed accept clean-up replaces the earlier clean-up clause
  (`restore::merge_outcome`). P4: the M8a.22 note on the built-in roster corrected.
  P5: follow-ups. `review.rs`'s `Medium` fallback now has a `debug_assert!` that the
  task has a review level.
- **T23.** P1: the `codex_project_config` doc comment restored. P2: the not-a-terminal
  hint names each command's flags (`--yes`/`--base` for accept, `--confirm <run id>` for
  reject and discard). M2's pre-check note: follow-ups.
- **Review E.** M2 (file size) below. M3: `run_info.rs`'s module doc now says an
  `Option` tolerates absence and a new non-`Option` field needs `#[serde(default)]` or a
  protocol bump. M4: `every_message_round_trips` carries `ClientKind::Mcp` and a headless
  `WindowInfo` with a `RunRef`. M5: unknown `[[orchestrator.models]]` entry keys and
  unknown budget rungs (scalar, empty table) are reported. M6: `run accept --base` on a
  base that has not moved must name the run's base (test
  `accept_with_a_base_that_is_not_the_runs_base_is_refused`). M7: rows in
  `docs/timing-budgets.md`. M8: follow-ups.
- **Review A.** M7 (`AGENTS.override.md`, `CLAUDE.local.md`) is already in the
  follow-ups for the user. M9 (containment): since F1c/F1d, checks, proofs and `setup`
  run confined on macOS and are denied the daemon socket, and workers' sandboxes deny
  it too, so agent-controlled code cannot reach the engine through the socket there.
  The engine still trusts a `ToolCall`'s `role` and `window_id` from any client that
  reaches the socket (no peer-pid check); with `worker_sandbox = false` or unconfined
  checks on Linux, that is the user's documented choice.
- **T25's m3 for `run_e2e_finish.rs`.** The restart unsets
  `ANTHREX_TEST_ABORT_AFTER_INTENT` (`RunHarness::unset_env`) instead of setting it
  blank.
- **F1c's report reads.** `run_e2e_basic` waits for `## t1:` with `report_with`;
  `run_e2e_finish` uses the text its deadline wait returned.
- **File sizes.** Split by responsibility: `proto/src/run_tests.rs` (fixtures to
  `run_tests_fixtures.rs`), `config/src/orchestrator.rs` (unknown-key reporting to
  `orchestrator/unknown.rs`), `config/src/orchestrator_tests.rs` (profile and
  confinement tests to `orchestrator_tests_confine.rs`), `run/model.rs` (rounds and gate
  records to `model_rounds.rs`, re-exported), `run/plan_tests.rs` (route tests to
  `plan_tests_routes.rs`), `cli/tests/support/run_harness.rs` (`RunWatcher` to
  `run_watcher.rs`, re-exported) and `fake-agent/tests/headless_support/mod.rs` (the
