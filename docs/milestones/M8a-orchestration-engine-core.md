...
**Where this brief and the spec disagree, left open for a ruling** (none is clearly stale against the spec; each is a deliberate brief decision that later briefs already build on)

- Salvage refs: the spec says `refs/anthrex/salvage/<run>/<task>` (§12.2, §17); decision 20 uses `…/<task>/<seq>` so that a second salvage of one task cannot hit a ref directory/file conflict. M9.5 builds on `<seq>`.
- The small-review switch: the spec says `review.small = "off"` (§9, §22.2); the brief has `[orchestrator] review_small = false`. M9.5 cites `review_small`.
- Restart: the spec says non-terminal runs come back `Paused` (§17); decision 45 keeps `halted`, `awaiting_approval` and `complete` as they were (spec §12.3 keeps an unanswered gate; nothing runs in the other two).
- What a reviewer is given: the spec says the diff and the check output (§9, §11.4 step 3); decision 35 gives `Base` and `Head` and the 40-line check summary, and `REVIEWER_CONTRACT` tells the reviewer to run `git diff` and tests. A Claude reviewer's `--allowedTools` (`mcp__anthrex__submit_review,Read,Glob,Grep`) has no `Bash`, and `--permission-prompts none` denies it, so as written a Claude reviewer cannot run either; a Codex reviewer under `-s read-only` can read git but cannot build. This one is internally contradictory as well as a spec difference.
