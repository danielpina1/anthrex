...
- Plan rules only planners are held to: `planner_task_cap`, scout evidence in `scout_refs`, no budgets from a planner, chain collapse, sub-planner scope.
...
23. **Rules for planners' edits**, in `run::orch::rules::check(run: &Run, touched: &BTreeSet<String>, source: &EditSource) -> Vec<PlanError>`, run after M8a's validation on every batch from the orchestrator (`edit_plan`) or a sub-planner (`submit_epic`). Plan files and the user's `run edit` keep M8a's rules only: a user's own plan is theirs, and M8a's tests use chains a planner would not.
    ...
    4. **Chains collapse.** A chain is two or more `pending` tasks `t1 → … → tk` in which each `t(i+1).deps == [t(i)]` exactly, each `t(i)` for `i < k` has `t(i+1)` as its only dependent among unfinished tasks, none is hub, all have the same kind (`code` or `docs`) and the same `modules_spanned` result `One(m)`, and the weights (S = 1, M = 3) sum to at most 3. A chain that includes a touched task is an error on its last task: `task <tk>: deps: <t1> -> … -> <tk> is a chain whose combined size is still M; make it one task (rule 7.3)`.
    ...
    Rule ids in `PlanError.rule`: `7.1.budget`, `7.1.evidence`, `5.1.cap`, `7.3.chain`, `2.epic`. *(Spec §7.1, §7.3, §12.1, §22.3.)*
...
pub fn chain_ending_at(run: &Run, task_id: &str) -> Option<Vec<String>>;                      // decision 23.4
...
13. A chain of tasks where each depends only on the previous one, and whose combined size is still M, is one task. Write it as one.
...
| `roster::{pick_reviewer, escalate}`, `run::globs::{validate_glob, intersects, names_literally}`, `modules_spanned` | Integration reviewer route; area and chain checks | M8a decisions 9, 11, 23 |
...
- `chain_of_s_s_s_is_refused_on_its_last_task`; `chain_of_m_s_is_accepted_because_it_exceeds_m`; `chain_through_a_hub_task_is_not_a_chain`; `fan_out_is_not_a_chain`; `chain_across_modules_is_not_a_chain`; `untouched_chain_is_not_reported`.
...
22. **§7.3's chain collapse is a planning rule with no check.** Resolved by decision 23.4, which checks it for planners' edits only.
