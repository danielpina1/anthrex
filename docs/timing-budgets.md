...
| Every raw run request but `Finish` | `REQUEST_WAIT` in `run_harness.rs` | `60s` | `run start`: its preflight's six git calls plus the id draw's and the settings scan's three, and (final fix batch F2) the base's `.codex` listing, at 5s each: 50s. Every other request is one engine step, whose inline effects are single fsynced file writes. | **Recorded.** |
...
| `a_swapped_private_dir_never_widens_the_grant` (M8a final fix batch F1c, I1) | `crates/daemon/tests/run_git_grant.rs` (`RACE_FOR`) | the race runs `3s` | Not a bound on anything the code does: the test calls `private_dir` in a loop for 3s while a thread swaps the directory for a link. Each call is a few `mkdir`/`lstat`/`realpath` syscalls, so thousands run (the pre-fix code lost the race 19 times in 6659 calls). The assertion is that no answer names another directory, and that at least one answered; a slow machine only makes fewer calls. | **Recorded.** No elapsed time is asserted. |
| `a_confined_command_that_times_out_is_killed_with_its_group` (M8a final fix batch F1c, I2) | `crates/daemon/tests/run_confine.rs` (`SLACK`) | `timeout (1s) + OUTPUT_GRACE + SLACK (2s)` | The same legal worst case as `run_exec.rs`'s `check_timeout_holds_under_an_output_flood`: the timeout, the group kill, then at most `OUTPUT_GRACE` of reading. `sandbox-exec` applies its profile and `exec`s the shell before the clock matters, so it adds only a spawn. | **Recorded.** Derived from the code's own `OUTPUT_GRACE`, with the file's standing `SLACK`. |
| `a_confined_check_cannot_open_an_app_it_built` (M8a final fix batch F1d, R1) | `crates/daemon/tests/run_confine_escape.rs` (`ESCAPE_WINDOW`) | the escape marker is watched for `3s` after the confined command returns | Not a bound on anything the code does. `open` returns before LaunchServices starts the app; unconfined (the red run on `6f0e852`), the app's marker appeared in well under a second. A deadline loop polled every 50ms returns as soon as the marker appears; when the confinement holds, it always waits the full window. The primary assertion does not depend on it: the confined `open` must not exit 0. | **Recorded.** An observation window for a negative result: a slow machine could only let a real escape go unseen, never fail a correct build. |
...
| The same test's holds | `run_e2e_sessions.rs` (`WINDOW_HOLD_MS`, `wait_for_window`) | `2000ms` on every `CreateWindow`'s `done` line; session 2's and the reviewer's tool calls wait until `run.json` records their window (an `sh` loop of 1500 × 0.2s, `300s`) | The hold only needs to outlast `fake-agent`'s start and exit (tens of ms). The gate (final fix batch F3, T25-N2; was a bare `4000ms` wait) returns as soon as the window is recorded: the hold plus a step and a save, under `stall_after_secs` (10s). Load can shrink the hold's margin over the start; the test then fails on its own check that session 1's round took no death (T25-N3), naming the race as not reproduced. | **Recorded.** |
...
### Recorded, from M8a's final fix batch F3 (2026-09-25)

| Test | Site | Bound | The code's own legal worst case | Status |
|---|---|---|---|---|
| `a_panicking_step_keeps_the_state_and_the_loop`, each request | `crates/daemon/src/run/driver/tests.rs` | `10s` | One reducer step (panicking, then caught) and a reply over a channel; no I/O. | **Recorded.** A hang guard: before the fix the loop died and the request was never answered. |
